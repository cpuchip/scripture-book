...
- **A *generative* redline pass (lenses read + redline the manuscript) does NOT work on the current substrate** — tried 2026-05-30; the lens fs sandbox can't reach the book source (it sees only `.spec/journal`-ish paths, not `src/chapters/`), so the panel produces generic advice or empties, not text-anchored edits. The brainstorm rails are evaluative (ideate on a binding question), not document-redline. Enabling the generative pass needs a substrate change — see the cross-project proposal `/.spec/proposals/substrate-multimodel-document-redline.md`. Until then, do the generative/voice work via a direct agent (e.g. the Gemini voicing pass), not the substrate panel.
