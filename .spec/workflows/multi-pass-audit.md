...
- **Two opencode models are unusable via the substrate — root cause diagnosed 2026-05-29** (gateway probe, `extension/smoke/test-glm-qwen-models.sh` + `test-glm-headroom.sh`). The v2 run's empties were not "flakiness"; they were deterministic incompatibilities:
  - **`qwen3.7-max` — hard error, never usable.** The gateway returns `Model qwen3.7-max is not supported for format oa-compat` for both streaming and non-streaming. It appears in the `/models` catalog but cannot be dispatched on the OpenAI-compat endpoint the substrate uses. **Do not assign it to any lens.**
  - **`glm-5` / `glm-5.1` — empty over streaming.** Both map to backend `frank/GLM-5.1` (a reasoning model). Non-streaming with adequate `max_tokens` works perfectly (real content, `finish_reason: stop`). But over **streaming** — which is how the substrate dispatches (`stream:true`, required since ES.6 to dodge the gateway's 125s idle-timeout) — the content never arrives: 0 content chars, 0 reasoning chars, no `finish_reason`, no `usage`. So GLM completes-but-empty via the substrate. **Avoid for substrate lenses until the gateway's GLM streaming is fixed.**
  - Both are now annotated in `stewards.model_pricing.notes` so they surface in the `/models` catalog browser with the caveat. The free models `deepseek-v4-flash` and `mimo-v2.5` stream reliably at $0 — a good default for fan-out. Still check each child's `status`/`maturity` after a run and re-fire any empties on a proven model.
