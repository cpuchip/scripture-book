...
| Ch 4 · Watched Until They Obey | clean (thesis threaded; trust gradient + agency each add) | KEEP — watching hub (gradient / instruments / steer-vs-run distinct); reused P6/P7 examples kept (reframed); Sabbath flag ✓resolved at Ch 11: **trimmed ¶30** (Abr 5:2 + Moses 3:2 preview → brief pointer; Ch 11 owns them); ¶46 seventh-day-review kept (distinct watching) | EDITED | uncommitted |
...
| Ch 9 · Hope and the Veil | longest chapter; EP re-walked the full Core Reframe (3 postures + 4 groups) | **compressed the 3-postures re-walk ¶53–57 (3¶ → 1¶)**; kept four-groups + the Group 2 engineering warning (¶61, the distinct payload) + Ch 10 bridge | EDITED | `1ce4fca` |
| Ch 10 · Softening What I Cannot Soften | clean (deliberate rebuild — gardening + AI-mirror; witnesses build) | ¶37 re-listed Ezekiel's verbs (¶19 has the quote) → **trimmed the re-list**; kept the room/gift synthesis | EDITED | uncommitted |
| Ch 11 · The Seventh Time | clean (Sabbath owner; thesis threaded; imago-Dei close restates as its mechanism) | KEEP — EP distinct (retros-during-sprint vs cessation; the pronouncement; fullness-inverts-economy) | Clean — no change | — |
| Ch 12 · From Consecration to Zion | clean (no-poor/shared-intent threaded; ¶41/¶43 deliberate bracket) | EP distinct (rebuilt — ward-council vs microservices/conductor; shared-intent; token consecration); **¶41 Jethro tier re-list trimmed** (Ch 7 lists them ×2) | EDITED | uncommitted |
| Epilogue · The Silent Loop | clean — special form (AI's voice); meditative refrain builds (software → God → book → you), like the coda | n/a (no EP) | KEEP as-is (coda precedent) | — |
| Afterword · How I Got Here | clean — chronology (distinct milestones) + Constant Principles coda (builds: pace → principles endure → industry → responsibility) | n/a (no EP) | Clean — no change | — |
| **— WALK COMPLETE —** | preface + P1–10 + eleven-step + Ch 0–12 + Epilogue + Afterword | **11 chapters edited, rest clean** | glossary + Recommended Study skipped (reference lists) | — |
