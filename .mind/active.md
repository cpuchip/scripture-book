...
> **2026-05-29 (latest) — v2 BATCH 2 (structural, partial).** Glossary reconciliation (T1.1 — Heart-Softening→yielding/mirror per Ch10; Zion→shared-intent ward-council, not microservices, per Ch12; Dross→removed the false "overfitting→compilation errors"). Reference page reworded to drop the "walk each step in turn" overpromise (T1.4 — chapters are thematically ordered; the italic tags are the cycle-placement aid). Ch8/Ch10 tags differentiated → *Atonement · Refinement* / *Atonement · Yielding*. **Still open in Batch 2:** T1.2 (tighten Ch9's engineering half) + a chapter-order/flow review Michael requested (why 8 and 10 are split by the long Ch9). **dist/ rebuild deferred until the order question settles.**
