...
We must also watch our instruments, not just the code. While publishing this book's own website, a deploy check came back 200 — success — seconds after the push, far too soon for the build to have finished. The success was real and meant nothing: that server answers every address with a friendly page, so the status code was measuring the server's politeness, not the artifact. The true witnesses were the content type and the byte count, and they said *not yet*. An instrument that cannot say no is not watching anything; before trusting a reading, ask what the gauge would show if the thing were false.
