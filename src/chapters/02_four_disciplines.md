...
Intent engineering fails when real autonomy begins. An agent can understand its purpose and still get caught in an infinite loop. We saw this when an autonomous research agent in our Postgres database looped on a single topic and burned through 230 million input tokens before we hit the emergency stop (Practice 3 opens with that story). The agent's intent was defined, but intent alone did not prevent it from drifting.
...
The industry has begun to feel how much the four disciplines leave out. In early 2026 a new phrase moved to the center of the conversation: *harness engineering*. Its slogan was *Agent = Model + Harness*. The model is the engine; the harness is everything else — the tools the agent may call, the guardrails that keep it safe, the feedback loops that let it self-correct, the observability that lets a human see what it did. In one published case, [LangChain lifted its coding agent](https://www.langchain.com/blog/improving-deep-agents-with-harness-engineering) [qr](https://www.langchain.com/blog/improving-deep-agents-with-harness-engineering) from outside a benchmark's top thirty to fifth place without touching the model at all, purely by rebuilding the harness. But a harness is the wrapper around the model, not a fifth altitude of *organize*, and the wrapper is built from the verbs the four disciplines never named.
