<!--
DRAFT — Part 1, Practice 4. Story-led (scar -> the ./context upgrade). NOT manuscript. Voice pass owed.
Scar: J.3 token-limit deaths -> engram compaction (MEMORY substrate Batch J/K, ~2026-05-13/14).
Upgrade: ./context library = download 300+ interconnected work repos so the agent can explore + trace
ripple (Michael's account this session). ./external_context = reference repos. YT-transcript-tool aside
= context precursor. Scripture D&C 88:119 VERIFIED gospel_get 2026-05-31. "Line upon line" referenced as the
Part 2 step, not quoted.
-->

# Pack the Context, Waste Nothing

The first instinct, when you want a good answer, is to give the model everything. The whole codebase. All the docs. The entire history. It feels generous, and it is exactly wrong. I learned this when I fanned a job out to six agents at once and packed each one's window full of context I thought might be useful. Four of the six died on the spot — they hit the token limit before they could do any work. I'd confused *having* the information with *using* it, and I'd drowned the very agents I was trying to equip. We ended up building a whole compaction layer just to dig back out.
...
That's the window half. Once the library exists, the discipline flips: into any given task's window goes only what that task needs — the two files, the one contract, the relevant prior decision. Everything else stays on the shelf. The context window is sacred space. The skill is not stuffing it; the skill is choosing.

The scripture names both halves in one verse. "Organize yourselves; *prepare every needful thing*; and establish a house... a house of order" ([D&C 88:119]). *Prepare every needful thing* is the library — make it ready and available. The *house of order* is the window — everything in its place, nothing crowding what matters. Understanding then comes the way scripture says it always comes: a little at a time, line upon line, drawn from a prepared store. ([Part 2: Line upon Line — the doctrine under this practice.])
...
<!--
PROVENANCE (migrate to provenance_part1.md P4):
- J.3 token deaths -> Batch K engrams: MEMORY substrate Batch J/K plans (~2026-05-13/14). VERIFY "4 of 6" if printed.
- ./context = 300+ interconnected work repos for exploration + ripple-tracing; ./external_context = reference repos. Source: Michael this session. VERIFY the "300+" figure with Michael (his claim).
- YT-transcript tool = context precursor (do NOT name the repo). Source: Michael this session + origins mining.
- D&C 88:119 — VERIFIED gospel_get 2026-05-31.
- "Line upon line" referenced as the Part 2 step, NOT quoted (avoid unverified quote).
- Eternal vs 2026 separated (windows may grow, changing the calculus).
- Voice flags: em-dash budget; "fog/shelf/sacred space" imagery density; confirm 300+ figure.
-->
