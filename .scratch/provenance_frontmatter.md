...
*   **Colophon agent descriptions rewritten 2026-05-27** — the previous version (drafted by Gemini in the initial pass) overclaimed each agent's adherence to the workspace harness. First rewrite replaced the bullets with detailed descriptions verified against the actual harness files, the activity log in the Preface, and workspace memory entries. **Second rewrite same day** trimmed the length by about half: the Colophon does not need to enumerate workspace directories or dwell on the Gemini failure mode; the Chapter 6 sidebar already does that work. Final bullets frame Copilot + Claude Code as the day-to-day partners and Gemini as the voicing / balancing / quick-structure partner, with a pointer to Chapter 6 for how the workspace is learning to work with the developing Gemini harness. Model identifier for Gemini (Gemini 3.5 Flash) added for symmetry with the Opus 4.7 identifiers on the other two bullets.
